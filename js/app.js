(() => {
    ...
    window.addEventListener("load", (function(e) {
        ...
        function counterAnimate(counter) {
            ...
            let duration = parseInt(counter.dataset.counter) ? parseInt(counter.dataset.counter) : 1e3;
            let startValue = parseInt(counter.innerHTML);
            let startPositon = 0;
            const step = timeStamp => {
                if (!startTimeStamp) startTimeStamp = timeStamp;
                const progress = Math.min((timeStamp - startTimeStamp) / duration, 1);
                counter.innerHTML = Math.floor(progress * (startPositon + startValue));
        ...
        if (playButton) playButton.addEventListener("click", (function(e) {
            ...
            document.querySelector(".video-block__video").setAttribute("src", `${newSrc}`);
            document.querySelector(".video-block__item").classList.add("_play");
        }));
    }));
    window["FLS"] = true;
    isWebp();
